...
if plot_s_d:
    ...
    threshold_clip = 0.3
    ds_clipped = np.array([])
    s_mins_clipped = np.array([])
    for i in range(len(ds)):
        if ds[i] < threshold_clip:
            ds_clipped = np.append(ds_clipped, ds[i])
            s_mins_clipped = np.append(s_mins_clipped, s_mins[i])
    correlation_matrix = np.corrcoef(ds_clipped,s_mins_clipped)

    fig = plt.figure()
    ax = fig.add_subplot(1, 1, 1)
    ax.tick_params(axis='both', which='major', labelsize=19)
    plt.loglog(ds, s_mins, '*')
    plt.xlabel(r'$|d+1|$', fontsize=23)
