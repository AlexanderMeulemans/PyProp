...
from layers.original_TP_layer import OriginalTPLayer

class OriginalDTPLayer(OriginalTPLayer):
    """ Original Difference target propagation, as discribed in
    Lee et al 2015"""

    def propagate_backward(self, upper_layer):
        if not isinstance(upper_layer, OriginalTPLayer):
        ...
        target_output = self.backward_nonlinearity(
            target_linear)
        activation_output = self.backward_nonlinearity(activation_linear)
        backward_output = self.forward_output + target_output - activation_output
        self.set_backward_output(backward_output)


class OriginalDTPLeakyReluLayer(OriginalDTPLayer):
...
class OriginalDTPLinearLayer(OriginalDTPLayer):
...
class OriginalDTPOutputLayer(OriginalDTPLayer):
...
class OriginalDTPLinearOutputLayer(OriginalDTPOutputLayer):
